...
```text
โจทย์ต้องการให้รับเลขจำนวนเต็มจำนวนหนึ่ง แล้วให้หาว่า ตั้งแต่เลขนั้น จนถึง 0 มีเลขไรบ้างที่หาร 10 ลงตัว

Input:
รับเลขจำนวนเต็มบวก 1 จำนวน

Output:
เลขตั้งแต่จำนวนที่ input ถึง 0 ที่หาร 10 ลงตัว มีเลขอะไรบ้าง และแสดงอวยู่ในบรรทัดเดียวกัน
```
...
```text
Step 1: รับเลขจำนวนเต็มบวก
Step 2: สร้างลูป for ใน range(input, -1, -1) เพื่อจะเช็กเลขทีละตัว
Step 3: กำหนดเงื่อนไข ว่าถ้า mod 10 ได้ 0 ให้ปรินท์ออกมา + sep = " "
```
...
```text
ใช้ตามแผนแรก แต่เปลี่ยนจาก sep เป็น end
```
...
```text
เลขหลักเยอะ อยากทดสอบว่าโปรแกรมจะค้างหรือไม่ทำงานไหม
```
...
```text
สอนว่าต้อง step = -1 ถึงจะเดินจากเลขมากไปน้อย
สอนว่าต้องใช้  end = " " ถึง output จะต่อกัน
```

คุณยังทำอะไรด้วยตนเอง

```text
ลูป for ปริ้นท์ f string {i}
```
